...
def generate_com_trajectory(footsteps, initial_com_pos, initial_com_vel, step_time, com_height, g=9.81):
    """
    使用LIP/ALIP模型的解析解，从落脚点序列生成CoM轨迹。
    ...
    for i in range(len(footsteps) - 1):
        ...
        while t < step_time:
            # 解析解公式
            cosh_t = np.cosh(omega0 * t)
            sinh_t = np.sinh(omega0 * t)
            
            pos_t = (current_com_pos - stance_foot_pos) * cosh_t + (current_com_vel / omega0) * sinh_t + stance_foot_pos
            vel_t = (current_com_pos - stance_foot_pos) * omega0 * sinh_t + current_com_vel * cosh_t
        ...
        cosh_T = np.cosh(omega0 * step_time)
        sinh_T = np.sinh(omega0 * step_time)
        
        next_com_pos = (current_com_pos - stance_foot_pos) * cosh_T + (current_com_vel / omega0) * sinh_T + stance_foot_pos
        next_com_vel = (current_com_pos - stance_foot_pos) * omega0 * sinh_T + current_com_vel * cosh_T
...
if __name__ == '__main__':
    ...
    plt.plot(com_pos_np[0, 0], com_pos_np[0, 1], 'gx', markersize=12, mew=2, label='Initial CoM')
    
    plt.title('CoM Trajectory Generation from Footsteps (LIP/ALIP Model)')
    plt.xlabel('X (m)')
    plt.ylabel('Y (m)')
    plt.legend()
    plt.grid(True)
