...
int main(int argc, char *argv[]) 
{
    try {
        ...
        std::vector<std::vector<std::vector<GRBVar>>> x_alip_vars(N_horizon, std::vector<std::vector<GRBVar>>(K_knots, std::vector<GRBVar>(4)));
        // u_ankle_vars[n][k]
        std::vector<std::vector<GRBVar>> u_ankle_vars(N_horizon, std::vector<GRBVar>(K_knots - 1));

        std::vector<std::vector<GRBVar>> v_ankle_vars(N_horizon, std::vector<GRBVar>(K_knots - 1));
        ...
        for (int n = 0; n < N_horizon; ++n) 
        {
            for (int k = 0; k < K_knots; ++k) 
            {
                for (int i = 0; i < 4; ++i) 
                {
                    x_alip_vars[n][k][i] = model.addVar(-GRB_INFINITY, GRB_INFINITY, 0.0, GRB_CONTINUOUS, "x_n" + std::to_string(n) + "_k" + std::to_string(k) + "_" + std::to_string(i));
                }
                if (k < K_knots - 1) {
                    u_ankle_vars[n][k] = model.addVar(-5.0, 5.0, 0.0, GRB_CONTINUOUS, "u_n" + std::to_string(n) + "_k" + std::to_string(k));
                    v_ankle_vars[n][k] = model.addVar(-5.0, 5.0, 0.0, GRB_CONTINUOUS, "v_n" + std::to_string(n) + "_k" + std::to_string(k));
                }
            }
        }
        ...
        for (int n = 0; n < N_horizon; ++n) 
        {
            for (int j = 0; j < 3; ++j) 
            {
                p_foot_vars[n][j] = model.addVar(-GRB_INFINITY, GRB_INFINITY, 0.0, GRB_CONTINUOUS, "p_n" + std::to_string(n+1) + "_" + std::to_string(j));
            }
        }
        ...
        for (int n = 0; n < N_horizon; ++n) 
        {
            for (int k = 0; k < K_knots - 1; ++k) 
            {
                ...
                {
                    model.addConstr(x_alip_vars[n][k+1][row] == gurobi_quicksum(A_d_mpc, row, x_alip_vars[n][k]) + B_d_mpc(row,0) * u_ankle_vars[n][k] + B_d_mpc(row, 1) * v_ankle_vars[n][k], "alip_dyn_n" + std::to_string(n) + "_k" + std::to_string(k) + "_r" + std::to_string(row));
                }
            }
        }
        ...
        for (int n = 0; n < N_horizon - 1; ++n) {
            ...
            for (int row = 0; row < 4; ++row) 
            {
                ...
                model.addConstr(x_alip_vars[n+1][0][row] == gurobi_quicksum(Ar_reset, row, x_alip_vars[n][K_knots-1]) + br_term, "alip_reset_n" + std::to_string(n) + "_r" + std::to_string(row));
            }
        }
        ...
        { // p_foot_vars[n] is p_{n+1}
            for (int i_reg = 0; i_reg < N_REGIONS; ++i_reg) 
            {
                ...
                for (int row_idx = 0; row_idx < F_mat.rows(); ++row_idx) 
                {
                    ...
                    model.addConstr( Fp_expr <= c_vec(row_idx) + M_big * (1.0 - mu_vars[n][i_reg]), "foothold_n" + std::to_string(n+1) + "_reg" + std::to_string(i_reg) + "_row" + std::to_string(row_idx)
                    );
                }
            }
            GRBLinExpr sum_mu = 0;
