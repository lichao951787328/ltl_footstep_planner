	...
	try:
		...
		for c in range(2,N):
			...
			if (c % 2 != 0):
				...
				term1_a = xn - (xc + p1_s[0]*footsteps[c-1][4] - p1_s[1]*footsteps[c-1][3])
				term2_a = yn - (yc + p1_s[0]*footsteps[c-1][3] + p1_s[1]*footsteps[c-1][4])

				m.addQConstr(term1_a*term1_a + term2_a*term2_a <= d1_s*d1_s)
				
				# Difficult terrain constraint
				term1_a_d = xn - (xc + p1_d[0]*footsteps[c-1][4] - p1_d[1]*footsteps[c-1][3])
				term2_a_d = yn - (yc + p1_d[0]*footsteps[c-1][3] + p1_d[1]*footsteps[c-1][4])
				m.addQConstr(-M*(1-H[c][1]) + term1_a_d*term1_a_d + term2_a_d*term2_a_d <= d1_d*d1_d)

				# term1_b = xn - (xc + p2[0])
				# term2_b = yn - (yc + p2[1])

				term1_b = xn - (xc + p2_s[0]*footsteps[c-1][4] - p2_s[1]*footsteps[c-1][3])
				term2_b = yn - (yc + p2_s[0]*footsteps[c-1][3] + p2_s[1]*footsteps[c-1][4])
				m.addQConstr(term1_b*term1_b + term2_b*term2_b <= d2_s*d2_s)

				# Difficult terrain constraint
				term1_b_d = xn - (xc + p2_d[0]*footsteps[c-1][4] - p2_d[1]*footsteps[c-1][3])
				term2_b_d = yn - (yc + p2_d[0]*footsteps[c-1][3] + p2_d[1]*footsteps[c-1][4])
				m.addQConstr(-M*(1-H[c][1]) + term1_b_d*term1_b_d + term2_b_d*term2_b_d <= d2_d*d2_d)
